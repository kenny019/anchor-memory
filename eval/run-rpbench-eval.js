...
import { loadRpBenchData } from './load-rpbench.js';
...
console.log('\n=== RPBench: 4-Configuration Comparison ===\n');

// Load data (fetches + generates probes on first run)
const rawData = await loadRpBenchData({ maxProbes: 5, llmCallFn: openRouterCall });
const conversations = rawData.conversations || [];

if (conversations.length === 0) {
    console.error('ERROR: No rpbench conversations loaded');
    process.exit(1);
}

// Build both heuristic and LLM episodes
const llmEpisodeCachePath = join(__dirname, 'data', 'rpbench-llm-episodes-cache.json');
let llmEpisodeCache = {};
...
for (const conv of conversations) {
    // Heuristic episodes (already built by loader)
